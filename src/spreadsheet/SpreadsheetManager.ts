...
export class SpreadsheetManager {
  ...
  constructor(private sheetId: string, private authToken: JWT) {
  ...
  async initialize() {
    ...
    logToPublicLog(`Available sheets: ${Object.keys(this.document.sheetsByTitle).join(', ')}`);
  ...
  async addMapping(merchantName: string, category: string) {
    ...
    await mapSheet.addRow({  // Non-null assertion
      merchant_name: merchantName,
      category: category,
      is_dynamic: "FALSE",
    });
    logToPublicLog(`Added new mapping: ${merchantName} -> ${category} with is_dynamic = FALSE`);
  }
}
