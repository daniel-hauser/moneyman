...
import { GOOGLE_SHEET_ID, TELEGRAM_API_KEY, TELEGRAM_CHAT_ID } from './config/AppConfig.js';
...
async function main() {
  try {
    ...
    const spreadsheetManager = new SpreadsheetManager(GOOGLE_SHEET_ID, authToken);
    ...
    mapRows.slice(0, 5).forEach((row, index) => {
        logToPublicLog(`Row ${index + 1}: merchant_name="${row.get('merchant_name')}", category="${row.get('category')}"`);
    });

    logToPublicLog('Creating TransactionClassifier...');
    const transactionClassifier = new TransactionClassifier(spreadsheetManager, TELEGRAM_API_KEY, TELEGRAM_CHAT_ID, classificationOptions);
